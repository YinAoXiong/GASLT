...
from signjoey.helpers import save_dir_to_zip, send_message
...
def train(cfg_file: str) -> None:
    ...
    for seed in range(131, 141):
        cfg["training"]["random_seed"] = seed
        cfg["training"]["model_dir"] = f"/data/yax_data/mm-jb/search_seed/{seed}"
        # set the random seed
        ...
        save_cfg_file = trainer.model_dir + "/config.yaml"
        ...
        best_path = "/home1/yinaoxiong/share/project/mm-jb/deformable_slt/best.json"
        with open(best_path, "r") as f:
            best_obj = json.load(f)
        dev_bleu = dev_best_result["valid_scores"]["bleu"]
        test_bleu = test_best_result["valid_scores"]["bleu"]
        if dev_bleu + test_bleu > best_obj["dev_bleu4"] + best_obj["test_bleu4"]:
            best_obj["path"] = cfg["training"]["model_dir"]
            best_obj["dev_bleu4"] = dev_bleu
            best_obj["test_bleu4"] = test_bleu
            with open(best_path, "w") as f:
                json.dump(best_obj, f, indent=4)
            message = f"find new best model !!! \n seed: {seed}\n dev bleu:{dev_bleu}\ttest bleu:{test_bleu}"
